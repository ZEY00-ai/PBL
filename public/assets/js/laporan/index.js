(function () {
    ...
    let page = parseInt(localStorage.getItem('laporan_page') || '1', 10);
    let perPage = parseInt(localStorage.getItem('laporan_perpage') || '10', 10);
    let query = '';

    if (pageSize) pageSize.value = perPage;

    function saveState() {
        localStorage.setItem('laporan_page', page);
        localStorage.setItem('laporan_perpage', perPage);
    }

    function render() {
        const q = query.trim().toLowerCase();
        const filtered = q ? allRows.filter(r => r.textContent.toLowerCase().includes(q)) : allRows.slice();
        ...
        if (total === 0) {
            ...
            filtered.forEach((r, i) => { if (i >= start && i < end) r.style.display = ''; });
    ...
    if (pageSize) pageSize.onchange = function () { perPage = parseInt(pageSize.value, 10); page = 1; render(); };
    if (clearBtn) clearBtn.onclick = function () { search.value = ''; query = ''; page = 1; render(); };
